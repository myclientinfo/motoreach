...
if($submit == "Crear Tabla"){
	$newtablequery = "CREATE TABLE `".$tablenament."` (\n";
		for ($j = 0; $j < count($values)-1; $j++){
			$field_length[$j] = str_replace('\\','',$field_length[$j]);
				$newtablequery .= "`".addslashes(utf8_decode(trim($values[$j],"'\n")))."` $field_type[$j]".(($field_length[$j] != "") ? "( $field_length[$j] )":"")." $field_null[$j] $field_extra[$j]".(($j != (count($values) - 1)) ? " ," : "")."\n";
			}
			$newtablequery .= " PRIMARY KEY ( `$key` )) ;";
			//echo $newtablequery;
...
foreach ($file_contents_line as $key => $val){
	$field1++; //skip field names or first line
		// Skip empty lines
	if (empty($val) || $field1 == 1) continue;
	...
	for ($j = 0; $j < count($values)-1; $j++)
	{
		$inserts .= addslashes(utf8_decode(trim(str_replace("~",",",$values[$j]),"'\n"))).(($j != (count($values) - 2)) ? "', '" : "");
	$val=$val1="";}
...
$inserts2 .= "(`";
	...
	for ($j = 0; $j < count($values)-1; $j++)
	{
		$inserts2 .= addslashes(utf8_decode(trim($values[$j],"'\n" ))).(($j != (count($values) - 2)) ? "`, `" : "");
	}
	$inserts2 .= "`)";
